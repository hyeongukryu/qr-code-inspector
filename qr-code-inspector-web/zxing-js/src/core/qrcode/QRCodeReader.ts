...
export default class QRCodeReader implements Reader {
  ...
  public decode(image: BinaryBitmap, hints?: Map<DecodeHintType, any>): Result {
    ...
    (window as any).noError = true;
    (window as any).finderPatternCount = 0;
    (window as any).darkQuietZoneCount = 0;
    (window as any).darkLuminances = [];
    (window as any).lightLuminances = [];
    (window as any).transformErrorCount = 0;
    ...
    if (other instanceof QRCodeDecoderMetaData) {
      result.putMetadata(ResultMetadataType.QR_CODE_VERSION, other.getVersion());
      result.putMetadata(ResultMetadataType.QR_CODE_NO_ERROR, (window as any).noError);
      result.putMetadata(ResultMetadataType.QR_CODE_IS_MIRRORED, other.isMirrored());
      result.putMetadata(ResultMetadataType.QR_CODE_FINDER_PATTERN_COUNT, (window as any).finderPatternCount);
      result.putMetadata(ResultMetadataType.QR_CODE_DARK_QUIET_ZONE_COUNT, (window as any).darkQuietZoneCount);
      result.putMetadata(ResultMetadataType.QR_CODE_AVERAGE_LIGHT_LUMINANCE,
        (((window as any).lightLuminances as number[]).reduce((a, b) => a + b, 0) / (window as any).lightLuminances.length));
      result.putMetadata(ResultMetadataType.QR_CODE_AVERAGE_DARK_LUMINANCE,
        (((window as any).darkLuminances as number[]).reduce((a, b) => a + b, 0) / (window as any).darkLuminances.length));
      result.putMetadata(ResultMetadataType.QR_CODE_TRANSFORM_ERROR_COUNT, (window as any).transformErrorCount);
    }
