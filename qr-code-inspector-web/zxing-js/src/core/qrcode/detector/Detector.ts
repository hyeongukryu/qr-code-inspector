...
export default class Detector {
  ...
  private resultPointCallback: ResultPointCallback | null = null;
  ...
  protected getResultPointCallback(): ResultPointCallback | null {
   ...
   * @throws FormatException if a QR Code cannot be decoded
   */
  public detect(hints: Map<DecodeHintType, any> | null | undefined): DetectorResult /*throws NotFoundException, FormatException*/ {
  ...
  protected processFinderPatternInfo(info: FinderPatternInfo): DetectorResult {
    ...
    const transformQuietZone: PerspectiveTransform =
      Detector.createTransformQuietZone(topLeft, topRight, bottomLeft, alignmentPattern, dimension);

    const bitsQuietZone: BitMatrix = Detector.sampleGrid(this.image, transformQuietZone, dimension + 8);

    let darkQuietZoneCount = 0;
    for (let i = 0; i < bitsQuietZone.getHeight(); i++) {
      for (let j = 0; j < bitsQuietZone.getWidth(); j++) {
        if (i < 4 || i >= bitsQuietZone.getHeight() - 4 || j < 4 || j >= bitsQuietZone.getWidth() - 4) {
          darkQuietZoneCount += bitsQuietZone.get(j, i) ? 1 : 0;
        }
      }
    }
    (window as any).darkQuietZoneCount = darkQuietZoneCount;
  ...
  private static createTransform(topLeft: ResultPoint,
    ...
    alignmentPattern: ResultPoint | null,
  ...
  private static createTransformQuietZone(topLeft: ResultPoint,
    topRight: ResultPoint,
    bottomLeft: ResultPoint,
    alignmentPattern: ResultPoint | null,
    dimension: number /*int*/): PerspectiveTransform {
    ...
    return PerspectiveTransform.quadrilateralToQuadrilateral(
      ...
      dimMinusThree + 4,
      3.5 + 4,
      sourceBottomRightX + 4,
      sourceBottomRightY + 4,
      3.5 + 4,
      dimMinusThree + 4,
      topLeft.getX(),
      topLeft.getY(),
